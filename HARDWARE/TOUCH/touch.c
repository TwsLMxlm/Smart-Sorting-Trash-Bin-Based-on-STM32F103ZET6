	...
	u8 i=0;	  	    
		...
		for(t=0;t<CT_MAX_TOUCH;t++)
		{
			if((tp_dev.sta)&(1<<t))
			{
				...
				{
					if(tp_dev.x[t]>100&&tp_dev.x[t]<(lcddev.width-100)&&tp_dev.y[t]<550&&tp_dev.y[t]>510)
					{
						LCD_Fill(100,510,(lcddev.width-100),550,LBBLUE);
						delay_ms(50);
						Menu_1();
					}
				  else if(tp_dev.x[t]>100&&tp_dev.x[t]<(lcddev.width-100)&&tp_dev.y[t]<615&&tp_dev.y[t]>565)
					{
						LCD_Fill(100,565,(lcddev.width-100),615,LBBLUE);
						delay_ms(50);
					  Menu_3();
					}
					else if(tp_dev.x[t]>100&&tp_dev.x[t]<(lcddev.width-100)&&tp_dev.y[t]<660&&tp_dev.y[t]>620)
					{
						LCD_Fill(100,620,(lcddev.width-100),660,LBBLUE);
						delay_ms(50);
					  Menu_2();
					}
				}
