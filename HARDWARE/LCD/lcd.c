...
void show_picture(u16 x,u16 y,u16 column,u16 row,unsigned short *p)
{
	u16 m,h;
	u16 *data=(u16*)p;
	for(h=0+y;h<row+y;h++)
...
void Show_Chinese(u16 x,u16 y,u16 index,u8 size,u16 pointcolor,u16 backcolor,u8 mode)
{
		u8 temp,t,t1;
	u16 y0=y;
	u8 csized=(size/8+((size%8)?1:0))*(size);	//一个汉字数据字节数，直接存就可比如size=24一个汉字是｛72数据｝，16是32
		for(t=0;t<csized;t++)
	{   												   
...
void Show_Chinese36(u16 x,u16 y,u16 index,u8 size,u16 pointcolor,u16 backcolor,u8 mode)
{
		u8 temp,t,t1;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);	//一个汉字数据字节数，直接存就可比如size=24一个汉字是｛72数据｝，16是32
		for(t=0;t<csize;t++)
	{   												   
		temp=hz36[index][t];			//换成相应的汉字数组得到点阵数据                          
		for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,pointcolor);
			else if(mode==0)
